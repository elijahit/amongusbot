...
class Logger(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        if log_channel is not None:
            ...
            log = discord.Embed(description=f"**Evento**: {member.mention} si è unito al server",
    ...
    async def on_member_remove(self, member):
        ...
        try:
            ...
            if log_channel is not None:
                ...
                account_age = str(account_age).split(" ")[0]
                joined_time = now.strftime("%d/%m/%Y %H:%M:%S")
                log = discord.Embed(description=f"**Evento**: {member.mention} è uscito al server",
    ...
    async def on_member_ban(self, guild, user):
        ...
        if log_channel is not None:
            ...
            log = discord.Embed(description=f"**Azione**: {user.mention} è stato bannato dal server.",
    ...
    async def on_member_unban(self, guild, user):
        ...
        if log_channel is not None:
            now = datetime.datetime.now()
            time = now.strftime("%d/%m/%Y %H:%M:%S")
            log = discord.Embed(description=f"**Azione**: {user.mention} è stato sbannato dal server.",
    ...
    async def on_message_delete(self, message):
        ...
        if log_channel is not None:
            log = discord.Embed(
                description=f"**Azione**: Messaggio di {message.author.mention} cancellato in {message.channel.mention}",
    ...
    async def on_message_edit(self, before, after):
        if not before.author.bot:
            ...
            log_channel = guild.get_channel(cfg.log)
            if log_channel is not None:
                log = discord.Embed(description=f"**Azione**: {before.author.mention} ha modificato il suo messaggio",
                                    colour=discord.Colour.from_rgb(66, 197, 245))
                ...
                log.add_field(name="Message",
                              value=f"[[Link](https://discordapp.com/channels/{before.guild.id}/{before.channel.id}/{before.id} 'Clicca qui per andare al messaggio editato.')]",
                              inline=True)
    ...
    async def on_guild_channel_create(self, channel):
        ...
        log_channel = guild.get_channel(cfg.log)
        if log_channel is not None:
            log = discord.Embed(description=f"**Azione**: Nuovo canale creato",
                                colour=discord.Colour.from_rgb(235, 174, 52))
    ...
    async def on_guild_channel_delete(self, channel):
        ...
        log_channel = guild.get_channel(cfg.log)
        if log_channel is not None:
            log = discord.Embed(description=f"**Azione**: Canale cancellato",
                                colour=discord.Colour.from_rgb(125, 235, 52))
            tipo = "Non disponibile"
            now = datetime.datetime.now()
            time = now.strftime("%d/%m/%Y %H:%M:%S")
            if str(channel.type) == "voice":
    ...
    async def on_member_update(self, before, after):
        if before.nick != after.nick:
            ...
            if log_channel is not None:
                now = datetime.datetime.now()
                time = now.strftime("%d/%m/%Y %H:%M:%S")
                log = discord.Embed(description=f"**Azione**: nickname di {before.mention} modificato",
                                    colour=discord.Colour.from_rgb(52, 159, 235))
        ...
        if before.roles != after.roles:
            ...
            if log_channel is not None:
                ...
                now = datetime.datetime.now()
                time = now.strftime("%d/%m/%Y %H:%M:%S")
                log = discord.Embed(description=f"**Azione**: ruoli di {before.mention} aggiornati",
                                    colour=discord.Colour.from_rgb(52, 235, 229))
                log.set_author(name=before, icon_url=before.avatar_url)
    ...
    async def on_guild_update(self, before, after):
        ...
        if log_channel is not None:
            ...
            if before.name != after.name:
                log = discord.Embed(description=f"**Azione**: nome server modificato",
                                    colour=discord.Colour.from_rgb(52, 159, 235))
                ...
                log = discord.Embed(description=f"**Azione**: afk-room server modificata",
                                    colour=discord.Colour.magenta())
                log.set_author(name=before.name, icon_url=before.icon_url)
                log.add_field(name="Vecchia stanza", value=f"{before.afk_channel.name} (`{before.afk_channel.id}`)",
                              inline=True)
                log.add_field(name="Nuova stanza", value=f"{after.afk_channel.name} (`{after.afk_channel.id}`)",
                              inline=True)
                log.add_field(name="Orario azione", value=time, inline=False)
                await log_channel.send(embed=log)
    ...
    async def on_invite_create(self, invite):
        ...
        if log_channel is not None:
            now = datetime.datetime.now()
            time = now.strftime("%d/%m/%Y %H:%M:%S")
            log = discord.Embed(description=f"**Azione**: {invite.inviter.mention} ha creato un invito",
                                colour=discord.Colour.from_rgb(235, 52, 128))
            ...
            log.add_field(name="Durata invito",
                          value=f"{f'{invite.max_age} s' if invite.max_age != 0 else 'Permanente'}", inline=True)
            log.add_field(name="Utilizzi",
                          value=f"{invite.uses}/{invite.max_uses if invite.max_uses != 0 else 'Infiniti'}", inline=True)
            log.add_field(name="Unione temporanea", value='Si' if invite.temporary is True else 'No', inline=True)
    ...
    async def on_invite_delete(self, invite):
        ...
        if log_channel is not None:
            now = datetime.datetime.now()
            time = now.strftime("%d/%m/%Y %H:%M:%S")
            log = discord.Embed(description=f"**Azione**: invito cancellato",
                                colour=discord.Colour.from_rgb(235, 52, 128))
            log.add_field(name="Codice invito", value=f"`{invite.code}`", inline=True)
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if log_channel is not None:
            ...
            if before.channel is None and after.channel is not None:  # Utente si è connesso
                log = discord.Embed(description=f"**Azione**: {member.mention} si è connesso a {after.channel.name}",
                                    colour=discord.Colour.from_rgb(3, 252, 32))
                log.set_author(name=member, icon_url=member.avatar_url)
                ...
                log = discord.Embed(description=f"**Azione**: {member.mention} disconnesso da {before.channel.name}",
                                    colour=discord.Colour.from_rgb(252, 3, 3))
                log.set_author(name=member, icon_url=member.avatar_url)
                ...
                log = discord.Embed(
                    description=f"**Azione**: {member.mention} spostato da {before.channel.name} a {after.channel.name}",
                    colour=discord.Colour.from_rgb(252, 186, 3))
                log.set_author(name=member, icon_url=member.avatar_url)
                log.add_field(name="Vecchio canale", value=f"{before.channel.name} (`{before.channel.id}`)",
                              inline=True)
